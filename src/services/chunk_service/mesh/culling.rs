...
pub fn calculate_viewable(chunk: &Chunk, pos: [usize; 3]) -> ViewableDirection {
    ...
    if pos[1] != world.as_ref().unwrap()[0].len() - 1 && is_offset_transparent(world.as_ref().unwrap(), pos, &chunk.blocks, [0, 1, 0]) {
    ...
    if pos[0] != world.as_ref().unwrap().len() - 1 && is_offset_transparent(world.as_ref().unwrap(), pos, &chunk.blocks, [1, 0, 0]) {
    ...
    if pos[2] != world.as_ref().unwrap()[0][0].len() - 1 && is_offset_transparent(world.as_ref().unwrap(), pos, &chunk.blocks, [0, 0, 1]) {
