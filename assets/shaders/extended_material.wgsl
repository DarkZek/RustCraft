...
fn vertex(vertex_no_morph: CustomVertexInput) -> CustomVertexOutput {
    ...
    let skylight_strength = (f32(vertex_no_morph.skylight.x) / 12.0) * chunk_material.skylight_strength;
    let skylight_color = vec3(1.0, 0.9529, 0.9215) * skylight_strength;

    let lighting_strength = vertex_no_morph.lighting.w;

    // Blend
    let final_lighting_strength = lighting_strength - skylight_strength; // Prevent divide by zero
